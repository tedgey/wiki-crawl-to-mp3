...
exports.generateFiles = async (names, topic) => {
  ...
  try {
    ...
    eventEmitter.emit('generateComplete', { status: 'progress', step: 'scrapeArticles', message: 'Scraping wiki articles.' });
    ...
    for (const name of names) {
      ...
      eventEmitter.emit('generateComplete', { status: 'progress', step: 'generateText', message: `Generating text for ${name}.` });
      const generatedText = await generateText(prompt);

      const textKey = `scripts/${topic}/${fileName}.txt`;
      await uploadToS3('make-my-pod', textKey, generatedText, 'text/plain');
      eventEmitter.emit('generateComplete', { status: 'progress', step: 'generateTextSuccess', message: `Text generated for ${name}.` });

      eventEmitter.emit('generateComplete', { status: 'progress', step: 'generateAudio', message: `Generating audio for ${name}.` });
      ...
      eventEmitter.emit('generateComplete', { status: 'progress', step: 'generateAudioSuccess', message: `Audio generated for ${name}.` });
    }

    // Step 3: Generate images
    eventEmitter.emit('generateComplete', { status: 'progress', step: 'generateImage', message: 'Generating images.' });
